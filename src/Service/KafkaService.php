...
class KafkaService
{
    ...
    public function send(string $name, array $data, int $partition = -1, int $timeout = self::DEFAULT_TIMEOUT): void
    {
    ...
    public function getPublicTopic(string $name): ProducerTopic
    {
        if (false === isset($this->topics[$name])) {
            $this->topics[$name] = $this->producer->newTopic($name);
        }
